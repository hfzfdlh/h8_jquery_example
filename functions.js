...
function fetchProduct(BASE_URL){
    ...
    function deleteProduct(id,BASE_URL){
        ...
        .done(res=>{
            Swal.fire({
                icon:'success',
                title: `Product ${res.name} Deleted`}
               )
            productView()
        })
        .fail(err=>{
  ...
  function deleteCategory(id,BASE_URL){
    ...
    $.ajax({
        url:`http://${BASE_URL}/categories/${+id}`,
        type:'DELETE',
        headers:{
            access_token:localStorage.access_token
        }
